...
spec = describe "Unit" $ do
  ...
  it "combines quantities - cup equivalent to ounce" $
    combineQuantities (mapFromList [(cup, ([1 :: Int], 8)), (ounce, ([2], 1))]) `shouldBe` mapFromList [(ounce, ([1, 2], ReadableQuantity (Just 2) Nothing))]
  it "combines quantities - cup less than ounce" $
    combineQuantities (mapFromList [(cup, ([1 :: Int], 4)), (ounce, ([2], 1))]) `shouldBe` mapFromList [(ounce, ([1, 2], ReadableQuantity (Just 1) (Just (ReadableFraction 1 2))))]
  it "combines quantities - teaspoons to tablespoons to cups" $
    combineQuantities (mapFromList [(teaspoon, ([1 :: Int], 2)), (tablespoon, ([2], 2)), (cup, ([3], 4))]) `shouldBe` mapFromList [(cup, ([1, 2, 3], ReadableQuantity (Just 4) (Just (ReadableFraction 1 4))))]
  it "retains quantities not in known conversions" $
    combineQuantities (mapFromList [(ounce, ([1 :: Int], 1)), (Unit "whole", ([2], 1))]) `shouldBe` mapFromList [(ounce, ([1], ReadableQuantity (Just 1) Nothing)), (Unit "whole", ([2], ReadableQuantity (Just 1) Nothing))]
