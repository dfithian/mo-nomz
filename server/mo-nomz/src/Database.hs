...
import Types
  ( Ingredient(..), Recipe(..), User(..), RecipeId, RecipeLink, RecipeName, UserId, Username
  , uncurry3
  )
...
selectRecipeIngredients :: Connection -> UserId -> [RecipeId] -> IO (Map RecipeId Recipe)
selectRecipeIngredients conn userId recipeIds = do
...
insertRecipe :: Connection -> UserId -> RecipeName -> Maybe RecipeLink -> [Ingredient] -> IO RecipeId
insertRecipe conn userId name link ingredients = withTransaction conn $ do
  [Only recipeId] <- returning conn "insert into nomz.recipe (user_id, name, link) values (?, ?, ?) returning (id)" [(userId, name, link)]
